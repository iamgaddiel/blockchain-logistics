...
export async function getFirstItem(collection: string, filterParams: string, relFields = "") {
    try {
        const firstItem = await pb.collection(collection).getFirstListItem(filterParams, {
            expand: relFields,
            '$cancelKey': collection
        });
        return firstItem
